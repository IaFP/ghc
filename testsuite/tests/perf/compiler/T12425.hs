...
instance (Total m, Monad m) => Functor (Result a m) where
...
instance Total (CondT a m)

instance (Total m, Monad m) => Functor (CondT a m) where
    fmap f (CondT g) = CondT (liftM (fmap f) g)
    {-# INLINE fmap #-}

instance (Total m, Monad m) => Applicative (CondT a m) where
    pure  = undefined
    (<*>) = undefined

instance (Total m, Monad m) => Monad (CondT a m) where
    return = undefined
    (>>=) = undefined
