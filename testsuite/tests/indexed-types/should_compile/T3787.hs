...
type instance Trampoline @@ s = TrampolineState @@ s
type instance Trampoline s @@ m = TrampolineState s @@ m
type instance Trampoline s m @@ r = m @@ TrampolineState s m r
...
type instance TrampolineState @@ s = ()
type instance TrampolineState s @@ m = ()
type instance TrampolineState s m @@ r = ()


instance (Total s, Total m, Functor s, Monad m) => Functor (Trampoline s m) where
   fmap = liftM

instance (Total s, Total m, Functor s, Monad m) => Applicative (Trampoline s m) where
   pure x = Trampoline (pure (Done x))
   (<*>) = ap

instance (Total s, Total m, Functor s, Monad m) => Monad (Trampoline s m) where
   return = pure
   t >>= f = Trampoline (bounce t >>= apply f)
      where apply f (Done x) = bounce (f x)
            apply f (Suspend s) = return (Suspend (fmap (>>= f) s))

instance (Total s, Total m, Functor s, ParallelizableMonad m) => ParallelizableMonad (Trampoline s m) where
...
instance (Total s, Functor s) => MonadTrans (Trampoline s) where
...
instance Total (NestedFunctor l r)
...
instance Total (SomeFunctor l r)
...
suspend :: (Total m, Total s, Monad m, Functor s) => s (Trampoline s m x) -> Trampoline s m x
...
pogoStickNested :: (Total s1, Total s2, Functor s1, Functor s2, Monad m) => 
...
coupleAlternating :: (Total s1, Total s2, Total m, Monad m, Functor s1, Functor s2) => 
...
coupleParallel :: (Total s1, Total s2, Total m, ParallelizableMonad m, Functor s1, Functor s2) => 
                  Trampoline s1 m x -> Trampoline s2 m y -> Trampoline (SomeFunctor s1 s2) m (x, y)
...
coupleNested :: (Total s1, Total s2, Total s0, Total m, Monad m, Functor s0, Functor s1, Functor s2) => 
...
seesaw :: (Total s1, Total s2, Total m, Monad m, Functor s1, Functor s2) => 
...
seesawParallel :: (Total s1, Total s2, Total m, ParallelizableMonad m, Functor s1, Functor s2) => 
                  (forall x y s t. (s ~ SomeFunctor s1 s2, t ~ Trampoline s m (x, y)) => s t -> t)
                  -> Trampoline s1 m x -> Trampoline s2 m y -> m (x, y)
seesawParallel resolve t1 t2 = pogoStick resolve (coupleParallel t1 t2)

resolveProducerConsumer :: forall a s s0 t t' m x. 
                           (Total s0, Total m, Functor s0, Monad m, s ~ SomeFunctor (TryYield a) (Await (Maybe a)), 
...
local :: forall m l r x. (Total r, Total m, Functor r, Monad m) => Trampoline r m x -> Trampoline (EitherFunctor l r) m x
...
out :: forall m l r x. (Total l, Total m, Functor l, Monad m) => Trampoline l m x -> Trampoline (EitherFunctor l r) m x
...
liftSink :: forall m a d x. (Total d, Total m, Monad m, AncestorFunctor a d) => Sink m a x -> Sink m d x
...
liftSource :: forall m a d x. (Total d, Total m, Monad m, AncestorFunctor a d) => Source m a x -> Source m d x
...
pipe :: forall m a a1 a2 x r1 r2. (Total a, Total m, Monad m, Functor a, a1 ~ SinkFunctor a x, a2 ~ SourceFunctor a x) =>
...
pipeP :: forall m a a1 a2 x r1 r2. (Total a, Total m, ParallelizableMonad m, Functor a, a1 ~ SinkFunctor a x, a2 ~ SourceFunctor a x) =>
         (Sink m a1 x -> Trampoline a1 m r1) -> (Source m a2 x -> Trampoline a2 m r2) -> Trampoline a m (r1, r2)
...
pipePS :: forall m a a1 a2 x r1 r2. (Total a, Total m, ParallelizableMonad m, Functor a, a1 ~ SinkFunctor a x, a2 ~ SourceFunctor a x) =>
...
getSuccess :: forall m a d x . (Total d, Total m, Monad m, AncestorFunctor a d)
...
get' :: forall m a d x . (Total d, Total m, Monad m, AncestorFunctor a d) => Source m a x -> Trampoline d m x
...
pour :: forall m a1 a2 d x . (Total d, Total m, Monad m, AncestorFunctor a1 d, AncestorFunctor a2 d)
...
pourMap :: forall m a1 a2 d x y . (Total d, Total m, Monad m, AncestorFunctor a1 d, AncestorFunctor a2 d)
...
pourMapMaybe :: forall m a1 a2 d x y . (Total d, Total m, Monad m, AncestorFunctor a1 d, AncestorFunctor a2 d)
...
-- and /sink2/.
tee :: forall m a1 a2 a3 d x . (Total d, Total m, Monad m, AncestorFunctor a1 d, AncestorFunctor a2 d, AncestorFunctor a3 d)
...
putList :: forall m a d x. (Total d, Total m, Monad m, AncestorFunctor a d) => [x] -> Sink m a x -> Trampoline d m [x]
putList [] sink = return []
...
getList :: forall m a d x. (Total d, Total m, Monad m, AncestorFunctor a d) => Source m a x -> Trampoline d m [x]
...
consumeAndSuppress :: forall m a d x. (Total d, Total m, Monad m, AncestorFunctor a d) => Source m a x -> Trampoline d m ()
...
whenNull :: forall a m. (Total m, Monad m) => m [a] -> [a] -> m [a]
...
putQueue :: forall m a d x. (Total d, Total m, Monad m, AncestorFunctor a d) => Seq x -> Sink m a x -> Trampoline d m [x]
