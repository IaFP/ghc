...
import GHC.Arr
import GHC.Base ( Applicative(..), Monad(..), Monoid, Maybe(..), NonEmpty(..), Splattable (..),
                  ($), (.), id, flip )
import GHC.Generics
...
class (Functor t, Foldable t) => Traversable t where
    ...
    -- Left 0
    --
    traverse :: (t @ f b, Splattable f) => (a -> f b) -> t a -> f (t b)
    ...
    -- Left 4
    --
    sequenceA :: Splattable f => t (f a) -> f (t a)
    ...
    -- power of 'Monad'.
    --
    mapM :: (t @ m b, Monad m, Splattable m) => (a -> m b) -> t a -> m (t b)
    ...
    -- Left 0
    --
    sequence :: Splattable m => t (m a) -> m (t a)
...
for :: (Traversable t, Splattable f, t @ f b) => t a -> (a -> f b) -> f (t b)
{-# INLINE for #-}
for = flip traverse

-- | 'forM' is 'mapM' with its arguments flipped. For a version that
-- ignores the results see 'Data.Foldable.forM_'.
forM :: (Traversable t, Splattable m, Monad m, t @ m b) => t a -> (a -> m b) -> m (t b)
{-# INLINE forM #-}
forM = flip mapM
...
--
mapAccumL :: forall t s a b. (Traversable t, Coercible a b, Coercible (t a) (t b), t @ StateL s a)
          => (s -> a -> (s, b)) -> s -> t a -> (s, t b) 
...
--
mapAccumR :: forall t s a b. (Traversable t, Coercible a b, Coercible (t a) (t b), t @ StateR s a)
          => (s -> a -> (s, b)) -> s -> t a -> (s, t b)
-- See Note [Function coercion] in Data.Functor.Utils.
...
-- @
fmapDefault :: forall t a b . (Traversable t, Coercible a b, Coercible (t a) (t b), t @ Identity b, t @ Identity a)
            => (a -> b) -> t a -> t b 
{-# INLINE fmapDefault #-}
-- See Note [Function coercion] in Data.Functor.Utils.
...
-- @
foldMapDefault :: forall t m a . (Traversable t, Monoid m, Coercible a (), Coercible (t a) (t ())
                                 , t @ (), t @ Const m a)
               => (a -> m) -> t a -> m
