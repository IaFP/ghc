...
module GHC.Types.Id.Make (
        ...
        mkDataConRep, updateDataConRep, mkDataConWorkId,
...
updateDataConRep us data_con
  ...
  where
    new_arg_tys = stableMergeScaledTypes (fmap unrestricted wfth) old_arg_tys
    ...
    new_dc_rep = do { wrap_args <- mapM (newLocal (fsLit "conrep2")) new_arg_tys
                    ; wrap_body <- mk_rep_app (wrap_args `zip` dropList eq_spec unboxers)
                                       initial_wrap_app
                    ...
                    -- the strictness signature (#14290).
                         ...
                         -- See Note [Activation for data constructor wrappers]
                          ...
                          wrap_unf = mkInlineUnfolding defaultSimpleOpts wrap_rhs
                          wrap_rhs = mkLams wrap_tvs $
                            mkLams wrap_args $
                            wrapFamInstBody tycon res_ty_args $
                            wrap_body
    ...
    ev_tys       = eqSpecPreds eq_spec ++ theta
    all_arg_tys  = map unrestricted ev_tys ++ new_arg_tys
    ev_ibangs    = map (const HsLazy) ev_tys

    wrap_arg_tys = (stableMergeTypes wfth theta) ++ (fmap scaledThing orig_arg_tys)
    wrap_arity   = count isCoVar ex_tvs + length wrap_arg_tys
              -- The wrap_args are the arguments *other than* the eq_spec
              -- Because we are going to apply the eq_spec args manually in the
              -- wrapper
