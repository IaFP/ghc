...
mkWDTcTyCon :: Name
          ...
          -> TyConFlavour        -- ^ What sort of 'TyCon' this represents
          -> TyCon
mkWDTcTyCon name binders res_kind scoped_tvs poly flav
  = let tc =
          TcTyCon { tyConUnique  = getUnique name
                  , tyConName    = name
                  , tyConTyVars  = binderVars binders
...
mkFamilyTyCon name binders res_kind resVar flav parent inj wfm
  = let tc =
          FamilyTyCon
            { tyConUnique  = nameUnique name
            , tyConName    = name
...
mkWDFamilyTyCon :: Name -> [TyConBinder] -> Kind  -- ^ /result/ kind
              -> Maybe Name -> FamTyConFlav
              -> Maybe Class -> TyCon
mkWDFamilyTyCon name binders res_kind resVar flav parent
  = let tc =
          FamilyTyCon
            { tyConUnique  = nameUnique name
            , tyConName    = name
...
isWDMirrorTyCon :: TyCon -> Bool
isWDMirrorTyCon (TcTyCon {isMirror = m}) = m
isWDMirrorTyCon (FamilyTyCon {isMirror = m}) = m
isWDMirrorTyCon tc = (tc `hasKey` wdTyConKey) || (isWDName $ tyConName tc)
...
-- | Is this 'TyCon' that for a class instance?
isClassTyConWithATs :: TyCon -> Bool
isClassTyConWithATs tc = isClassTyCon tc && not (null $ tyConATs tc)
